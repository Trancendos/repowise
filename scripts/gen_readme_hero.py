...
def card(x, y, w, h, label, kicker):
    add(
        f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="{R}" fill="{SURFACE}" stroke="{BORDER}" stroke-width="1"/>'
    )
...
add(
    f'<text x="{PAD}" y="{50}" class="h1">One index. Everything your agent and your team keep asking for.</text>'
)
add(
    f'<text x="{PAD}" y="{80}" class="sub">Index once, in seconds, no LLM required. Every answer below is already computed.</text>'
)
...
# KPI chips
kpis = [
    ("Defect risk", "7.5", "/10", WARNING),
    ("Maintainability", "8.6", "/10", SUCCESS),
    ("Perf risks", "268", "", PLUM),
]
kx = ax + 22
for name, val, unit, col in kpis:
    add(
        f'<rect x="{kx}" y="{ay+46}" width="146" height="58" rx="9" fill="{INSET}" stroke="{BORDER}"/>'
    )
    add(f'<text x="{kx+12}" y="{ay+66}" class="chiplbl">{esc(name)}</text>')
    add(
        f'<text x="{kx+12}" y="{ay+93}" class="chipval" fill="{col}">{val}<tspan class="chipunit">{unit}</tspan></text>'
    )
    kx += 156

# trust banner
add(
    f'<rect x="{ax+22}" y="{ay+116}" width="{aw-44}" height="34" rx="8" fill="{TRUST_BG}" stroke="{TRUST_BD}"/>'
)
add(f'<circle cx="{ax+40}" cy="{ay+133}" r="4.5" fill="{SUCCESS}"/>')
add(
    f'<text x="{ax+54}" y="{ay+138}" class="trust">17 of the 20 lowest-health files were bug-fixed in the last 6 months, <tspan font-weight="700">3.63× the 23% baseline</tspan></text>'
)
...
fx = ax + 490
findings = [
    ("CHANGE ENTROPY", "−3.0", ERROR),
    ("NESTED COMPLEXITY", "−2.4", ERROR),
    ("CHURN RISK", "−1.8", WARNING),
    ("CO-CHANGE SCATTER", "−1.5", WARNING),
...
for name, delta, col in findings:
    ...
    add(
        f'<text x="{fx+226}" y="{fy+16}" class="find" fill="{col}" text-anchor="end" font-weight="700">{delta}</text>'
    )
    fy += 28
...
add(
    f'<clipPath id="gclip"><rect x="{bx+18}" y="{by+46}" width="{bw-36}" height="240" rx="9"/></clipPath>'
)
...
for nx, ny, _nr, _col, ci, is_hub in nodes:
    ...
    hx, hy = gcent[ci]
    add(
        f'<line x1="{nx:.1f}" y1="{ny:.1f}" x2="{hx}" y2="{hy}" stroke="{HAIR_F}" stroke-width="1"/>'
    )
for i in range(len(gcent)):
    for j in range(i + 1, len(gcent)):
        if rng.random() < 0.75:
            add(
                f'<line x1="{gcent[i][0]}" y1="{gcent[i][1]}" x2="{gcent[j][0]}" y2="{gcent[j][1]}" stroke="{HAIR_S}" stroke-width="1.6"/>'
            )
...
for i, v in enumerate(bars):
    ...
    add(
        f'<rect x="{cx0+30+i*bw_:.1f}" y="{base-hgt:.1f}" width="{bw_-3:.1f}" height="{hgt:.1f}" rx="2.5" fill="{col}" opacity="0.82"/>'
    )
...
add(
    f'<text x="{cx0+30}" y="{cy0+276}" class="cap">hotspots · ownership · co-change · bus factor</text>'
)
...
for frac, kind in lines:
    if kind == 1:
        add(
            f'<rect x="{dx+34}" y="{ly-4}" width="{(dw-68)*frac:.0f}" height="11" rx="3" fill="{PLUM}" opacity=".85"/>'
        )
        ly += 22
    elif kind == 2:
        add(
            f'<rect x="{dx+34}" y="{ly-3}" width="{(dw-68)*frac:.0f}" height="9" rx="3" fill="{ACCENT}" opacity=".55"/>'
        )
        ly += 20
    else:
        add(
            f'<rect x="{dx+34}" y="{ly}" width="{(dw-68)*frac:.0f}" height="6" rx="3" fill="{DOCLINE}"/>'
        )
        ly += 16
add(f'<rect x="{dx+dw-116}" y="{dy+56}" width="86" height="20" rx="10" fill="{PILL_BG}"/>')
add(
    f'<text x="{dx+dw-73}" y="{dy+70}" class="pill" fill="{SUCCESS}" text-anchor="middle">fresh · v6</text>'
)
outcome(dx, dy + RH - 46, dw, "Why does auth work this way?")
add(
    f'<text x="{dx+22}" y="{dy+RH-25}" class="outsub">Answered from the wiki, not from 12 file reads.</text>'
)
...
for txt, col in badges:
    wpx = 9 + len(txt) * 6.6
    add(
        f'<rect x="{bxp}" y="{ey+122}" width="{wpx:.0f}" height="21" rx="10.5" fill="{SURFACE}" stroke="{BORDER_S}"/>'
    )
    add(
        f'<text x="{bxp+wpx/2:.0f}" y="{ey+136}" class="pill" fill="{col}" text-anchor="middle">{esc(txt)}</text>'
    )
    bxp += wpx + 8
add(
    f'<text x="{ex+34}" y="{ey+186}" class="cap">Delivered back at session start, and the moment</text>'
)
add(
    f'<text x="{ex+34}" y="{ey+202}" class="cap">your agent edits a file the decision governs.</text>'
)
