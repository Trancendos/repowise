...
async def _seed_monorepo(session):
    ...
    nodes = [
        {
            "node_id": f,
            "node_type": "file",
            "language": "python" if f.endswith(".py") else "typescript",
            "symbol_count": 1,
        }
        for f in files
    ]
    ...
    edges = [
        # Within container
        {
            "source_node_id": "packages/core/ingestion/parser.py",
            "target_node_id": "packages/core/ingestion/graph.py",
            "edge_type": "imports",
        },
        # Cross container
        {
            "source_node_id": "packages/web/lib/api.ts",
            "target_node_id": "packages/core/persistence/models.py",
            "edge_type": "imports",
        },
        # To externals
        {
        ...
        {
            "source_node_id": "packages/web/app/page.tsx",
...
async def test_build_l1_lists_externals_and_user(async_session):
    ...
    assert any(
        r.source_id == "person:user" and r.target_id == view.system.id for r in view.relations
    )
...
async def _seed_single_package(session):
    ...
    nodes = [
        {"node_id": f, "node_type": "file", "language": "python", "symbol_count": 2} for f in files
    ]
    # An unresolved-import target — must never become a file/component.
    nodes.append(
        {
            "node_id": "external:@/lib/thing",
            "node_type": "file",
            "language": "python",
            "symbol_count": 0,
        }
    ...
    session.add(
        GitMetadata(repository_id=repo.id, file_path="src/acme/core/engine.py", is_hotspot=True)
    )
    session.add(
        DeadCodeFinding(
            repository_id=repo.id,
