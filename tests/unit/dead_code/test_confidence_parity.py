...
def test_ts_confidence_floor_matches_python_risk_cap() -> None:
    ...
    medium_match = re.search(r"DEAD_CODE_CONFIDENCE\s*=\s*\{[\s\S]*?MEDIUM:\s*([0-9.]+)", content)
...
def test_ts_confidence_high_matches_python_safe_threshold() -> None:
    ...
    high_match = re.search(r"DEAD_CODE_CONFIDENCE\s*=\s*\{[\s\S]*?HIGH:\s*([0-9.]+)", content)
...
def test_ts_risk_factor_labels_match_python_blurbs() -> None:
    ...
    block = re.search(r"DEAD_CODE_RISK_FACTOR_LABELS[^=]*=\s*\{(.*?)\n\};", content, re.DOTALL)
