...
def _is_file_node(node: dict) -> bool:
    ...
    nid = node.get("id")
    return (
        isinstance(nid, str) and nid.startswith("file:") and isinstance(node.get("filePath"), str)
    )
...
def check_summaries_restate_filename(nodes: list[dict]) -> list[Finding]:
    ...
    for node in nodes:
        ...
        content = [
            w for w in _words(summary) if w not in _GENERIC_SUMMARY_WORDS and w not in name_tokens
        ]
        if not content:
            findings.append(
                Finding(
